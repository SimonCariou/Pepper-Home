...
public class ApiCallExecutorFr extends BaseQiChatExecutor {
    private QiContext mqiContext;
    private ApiClient apiclient;
    private Boolean lightStatus = false;
    private String TAG = "PepperHome_ApiCallExecutorFr";
    private Animate animateDab;

    //flag to say if the animation have already been built
    private Boolean objBuilt = false;

    public ApiCallExecutorFr(QiContext qictxt, ApiClient apiclient) {
        super(qictxt);
    ...
    public void runWith(List<String> params) {
        ...
        if(params.get(0).toLowerCase().equals("allume")) {
            lightStatus = true;
        }

        else if (params.get(0).toLowerCase().contains("tein")) {
            lightStatus = false;
        }
